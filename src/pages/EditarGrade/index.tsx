...
import { useGetGrade, useUpdateGrade } from "src/hooks/useGrade";
...
const EditarGrade = () => {
  ...
  const idGrade = idGradeString ? parseInt(idGradeString) : undefined;
  const { data: grade } = useGetGrade(idGrade);
  const { showSnackbar } = useSnackbar();
  const [e1, setE1] = useState<Dayjs | undefined>(
    dayjs(grade?.horarioEntradaInicial, "HH:mm:ss")
  );
  const [e1Error, setE1Error] = useState(false);
  const [e1Bloqueio, setE1Bloqueio] = useState(grade?.bloqueioEntradaInicial);
  const [e1BloqueioError, setE1BloqueioError] = useState(false);
  const [s1, setS1] = useState<Dayjs | undefined>(
    dayjs(grade?.horarioSaidaInicial, "HH:mm:ss")
  );
  const [s1Error, setS1Error] = useState(false);
  const [s1Bloqueio, setS1Bloqueio] = useState(grade?.bloqueioSaidaInicial);
  const [s1BloqueioError, setS1BloqueioError] = useState(false);
  const [e2, setE2] = useState<Dayjs | undefined>(
    dayjs(grade?.horarioEntradaFinal, "HH:mm:ss")
  );
  const [e2Error, setE2Error] = useState(false);
  const [e2Bloqueio, setE2Bloqueio] = useState(grade?.bloqueioEntradaFinal);
  const [e2BloqueioError, setE2BloqueioError] = useState(false);
  const [s2, setS2] = useState<Dayjs | undefined>(
    dayjs(grade?.horarioSaidaFinal, "HH:mm:ss")
  );
  const [s2Error, setS2Error] = useState(false);
  const [s2Bloqueio, setS2Bloqueio] = useState(grade?.bloqueioSaidaFinal);
  const [s2BloqueioError, setS2BloqueioError] = useState(false);
  const [dias, setDias] = useState({
    seg: grade?.seg !== null && grade?.seg !== undefined ? grade.seg : false,
    ter: grade?.ter !== null && grade?.ter !== undefined ? grade.ter : false,
    qua: grade?.qua !== null && grade?.qua !== undefined ? grade.qua : false,
    qui: grade?.qui !== null && grade?.qui !== undefined ? grade.qui : false,
    sex: grade?.sex !== null && grade?.sex !== undefined ? grade.sex : false,
    sab: grade?.sab !== null && grade?.sab !== undefined ? grade.sab : false,
    dom: grade?.dom !== null && grade?.dom !== undefined ? grade.dom : false,
  });
  const [diasError, setDiasError] = useState(false);

  const { mutate: updateMutate, isPending: isAtualizando } = useUpdateGrade();
      ...
      <FormGroup>
        ...
        <Stack direction={"row"} sx={{ mt: "1.5rem" }}>
          <Button
            ...
            onClick={() => navigate(`/listar-grades/${idGrade}`)}
